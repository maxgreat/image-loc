...
def ResNeXtVLAD(pretrained=True, vlad_clusters=64):
    base = torchvision.models.resnext50_32x4d(pretrained=pretrained)
    layers = nn.Sequential(*list(base.children())[:-2])
    model = nn.Module()
    model.add_module('encoder', layers)
